...
bigdf <- readRDS(".../data/gse108989_filtered_data.rds")

bigdf$celltype_group <- factor(as.character(bigdf$celltype_group), levels = c("CD4", "CD8"))

table(bigdf$celltype_group, bigdf$Patient)

colnames(bigdf)[colnames(bigdf) == "nCount_pre"] <- "n_counts"
colnames(bigdf)[colnames(bigdf) == "Patient_ID"] <- "Patient"
colnames(bigdf)[colnames(bigdf) == "celltype_group"] <- "Group"
...
metacols <- colnames(bigdf)[(ncol(bigdf)-6):ncol(bigdf)]


genes_keep <- c("APMAP", "ARAP2", "ARHGEF3", "ARID5A", "ATP1B3", "BIRC6", "BTG3", "BTN3A1", "CARD16",
                "CCSER2", "CD48", "CD5", "CD58", "CMTM6", "CMTM7", "CYTH1", "DHRS7", "EVI2A", "FAS", "GIT2",
                "GNAI3", "HNRNPLL", "ICAM3", "IGFLR1", "IL21R", "LBR", "MAP2K3", "MAT2A", "MVP", "NABP1", "NASP",
                "NCOA3", "NEDD9", "NSD3", "P2RY10", "PRNP", "PSTPIP1", "PTK2B", "RAPGEF6", "RHOG", "RUNX3",
                "SH2D1A", "SQSTM1", "SRI", "SRPK2", "STK17A", "TFRC", "TNFAIP8", "VMP1", "ZNF292")
...
pats_keep <- c("P0123", "P0215", "P0309", "P0411", "P0413", "P0701", "P0825", "P0909", "P1012", "P1212", "P1228")
...
sigpair_mat <- data.frame(b = 1:B)
for (k in 1:length(gene_pairs)){
  sigpair_mat[[gene_pairs[k]]] <- 0
}
sigpair_mat <- sigpair_mat[, -1]
...
for (b in 1:B){
  ...
  num_hubs <- 7
  hub_genes <- sample(genes_keep, num_hubs)
  pairmat <- NULL
  for (h in hub_genes) {
    targets <- setdiff(genes_keep, h) %>% sample(size = 20)  # connect to ~25% of other genes
    for (j in targets) {
      pairmat <- rbind(pairmat, c(h, j))
    }
  }
  
  pairmat <- rbind(pairmat, pairmat[, c(2,1)])
  
  pairmat_bind <- apply(pairmat, 1, function(a) paste(a[1], "_", a[2], sep=""))
  
  sigpair_mat[b, intersect(colnames(sigpair_mat), pairmat_bind)] <- 1
  
  
  for (i in 1:length(BATCH_copula$copula_list)){
    
    BATCH_copula$copula_list[[i]] <- BATCH_copula_ignoring_group$copula_list[[ceiling(i/2)]]
    
    if (i %% 2 == 0){
      
      for (nrw in 1:nrow(pairmat)){
        
        gn1 <- pairmat[nrw, 1]
        gn2 <- pairmat[nrw, 2]
        
        BATCH_copula$copula_list[[i]][gn1, gn2] <- BATCH_copula$copula_list[[i]][gn1, gn2] + 0.2
  ...
  for (method_idx in 1:(length(methods_use)-1)){
    ...
    if (methods_use[method_idx] == "CoCoA"){
      ...
      for (k in 1:length(gene_pairs)){
        ...
        tempdf$Group_bern <- 1*(tempdf$Group == "CD8")
    ...
    if (methods_use[method_idx] == "scDECO"){
      ...
      for (k in 1:(length(gene_pairs)-1200)){
        print(k)
        gene1 <- strsplit(gene_pairs[k], "_", 2)[[1]][1]
        gene2 <- strsplit(gene_pairs[k], "_", 2)[[1]][2]
        ...
        mcmc.out <- scdeco.cop(y=tempdf[, c("y1", "y2")], x=1*(tempdf$Group == "CD8"), marginals=c("ZINB", "ZINB"),
...
for (j in 1:length(dataset_list)){
  ...
  b <- as.integer(str_extract_all(dataset_name, "\\d+")[[1]][2])
