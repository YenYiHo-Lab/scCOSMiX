...
tau1s_use <- seq(0, 0.55, length=2) # 10
n_patients <- 10
      ...
      for (method_idx in 1:(length(methods_use)-1)){
        ...
        if (methods_use[method_idx] == "CSCORE"){
          ...
          what <- csCORE_pval(
                ...
                metacols = c("celltype", "patientID", "logn_counts"),
                groupcol = "celltype",
        ...
        if (methods_use[method_idx] == "CoCoA"){
          ...
          tempdf$celltype_bern <- 1*(tempdf$celltype == "CD8")
          
          tempdf$y1.norm <- gamlss::gamlss(formula = y1 ~ celltype + offset(logn_counts), 
                                           sigma.formula = ~ celltype,
                                           data = tempdf, family = gamlss.dist::NBI, trace=FALSE)$residuals
          
          tempdf$y2.norm <- gamlss::gamlss(formula = y2 ~ celltype + offset(logn_counts), 
                                           sigma.formula = ~ celltype, 
                                           data = tempdf, family = gamlss.dist::NBI, trace=FALSE)$residuals
          ...
          cocoa_df = data.frame(X = tempdf$y1.norm, Y = tempdf$y2.norm, Z=tempdf$celltype_bern)
        ...
        if (methods_use[method_idx] == "CNM.full"){
          ...
          zcor_matrix <- model.matrix(~ celltype_bern, data = tempdf)
        ...
        if (methods_use[method_idx] == "scDECO"){
          ...
          mcmc.out <- scdeco.cop(y=ourdf[, c("y1", "y2")], x=1*(ourdf$celltype == "CD8"), marginals=c("ZINB", "ZINB"), 
